...
-- https://fishlake-scripts.ru/resources/8/
function imgui.TextColoredRGB(text, id)
	...
	if openChat then
		...
		if imgui.BeginPopupContextItem() then
			if imgui.Button(u8'Edit', imgui.ImVec2(-1, 0)) then editChatLineId = id imgui.StrCopy(editChatLineColor, messages[id].color:match('{(.+)}')) imgui.StrCopy(editChatLine, messages[id].text) imgui.StrCopy(editChatLineTime, messages[id].timestamp:match('%[(.+)%]')) imgui.OpenPopup('EditChatLine1') end
			if imgui.Button(u8'Copy to clipboard') then setClipboardText(u8:decode(messages[id].text)) imgui.CloseCurrentPopup() end
			if imgui.Button(u8'Copy to chat field', imgui.ImVec2(-1, 0)) then imgui.StrCopy(inputChat, messages[id].text) imgui.CloseCurrentPopup() end
			if imgui.Button(u8'Delete', imgui.ImVec2(-1, 0)) then table.remove(messages, id) setup_current_scroll = setup_current_scroll - imgui.GetTextLineHeightWithSpacing() imgui.CloseCurrentPopup() end
			if imgui.BeginPopupModal('EditChatLine1', nil, imgui.WindowFlags.AlwaysAutoResize + imgui.WindowFlags.NoTitleBar) then
				local width = imgui.GetWindowWidth()
				local text_width = imgui.CalcTextSize(u8'Редактирование строки #'..editChatLineId)
				imgui.SetCursorPosX( width / 2 - text_width .x / 2 )
				imgui.Text(u8'Редактирование строки #'..editChatLineId)
				imgui.Separator()
				imgui.NewLine()
				imgui.Text(u8'Текст:')
				imgui.PushItemWidth(555)
				imgui.InputText('##editchatline', editChatLine, ffi.sizeof(editChatLine) - 1)
				imgui.PopItemWidth()
				imgui.SetCursorPosX(98)
				imgui.Text(u8'Цвет строки:') imgui.SameLine(400) imgui.Text(u8'Время:')
				imgui.SetCursorPosX(82.5)
				imgui.PushItemWidth(120)
				imgui.InputText('##editchatlinecolor', editChatLineColor, ffi.sizeof(editChatLineColor) - 1) 
				imgui.PopItemWidth()
				imgui.SameLine(78.75 + 286)
				imgui.PushItemWidth(120)
				imgui.InputText('##editchatlinetime', editChatLineTime, ffi.sizeof(editChatLineTime) - 1) 
				imgui.PopItemWidth()
				imgui.SetCursorPosX(82.5)
				if imgui.Button(u8'Применить', imgui.ImVec2(200, 0)) then
				...
				if imgui.Button(u8'Закрыть', imgui.ImVec2(200, 0)) then imgui.CloseCurrentPopup() end
				imgui.EndPopup()
			end
			imgui.EndPopup()
		end
...
local chat = imgui.OnFrame(function() return #messages > 0 and not isPauseMenuActive() and sampIsChatVisible() and not sampIsScoreboardOpen() and showChat end, function()
	...
	if openChat then
		...
		if imgui.InputText("##inputtext", inputChat, ffi.sizeof(inputChat) - 1, imgui.InputTextFlags.CallbackHistory + imgui.InputTextFlags.CallbackCompletion, TextEditCallback) then sampSetChatInputText(u8:decode(ffi.string(inputChat))) end
		imgui.PopStyleColor()
...
local settingsWindow = imgui.OnFrame(function() return enableSettingsWindow and enableSettingsWindow[0] end, function()
	...
	imgui.Begin(u8'Chat MImGui | Настройки | Автор: #Northn', enableSettingsWindow)
	local width = imgui.GetWindowWidth()
	local text_width = imgui.CalcTextSize(u8'Взаимодействие со шрифтом:')
	imgui.SetCursorPosX( width / 2 - text_width .x / 2 )
	imgui.Text(u8'Взаимодействие со шрифтом:')
	local font = imgui.GetIO().Fonts.Fonts.Data[0]
	imgui.Text(u8'Текущий шрифт: %s', font:GetDebugName())
	if imgui.Combo(u8'Выбрать шрифт', fontSelected, fontsArray, #fonts) then
		fontChanged = true
		data.values.font_name = fonts[fontSelected[0] + 1]
	end
	if imgui.SliderInt(u8'Размер шрифта', fontSize, 4, 30) then
		fontSizeChanged = true
		data.values.font_size = fontSize[0]
	end
	if imgui.SliderInt(u8'Количество строк', linesCount, 4, 72) then
		chatLinesCount = linesCount[0]
		data.values.line_count = linesCount[0]
	end
	imgui.Separator()
	local text_width = imgui.CalcTextSize(u8'Взаимодействие с цветами:')
	imgui.SetCursorPosX( width / 2 - text_width .x / 2 )
	imgui.Text(u8'Взаимодействие с цветами:')
	imgui.Text(u8'Цвет заднего фона:')
	imgui.SameLine(nil, 3)
	if imgui.ColorEdit4(u8'Цвет заднего фона чата', colorChatBG, imgui.ColorEditFlags.NoInputs + imgui.ColorEditFlags.NoLabel) then
	...
	imgui.Text(u8'Цвет поля ввода:')
	imgui.SameLine(nil, 3)
	if imgui.ColorEdit4(u8'Цвет поля ввода', colorChatInputBG, imgui.ColorEditFlags.NoInputs + imgui.ColorEditFlags.NoLabel) then
	...
	imgui.Text(u8'Цвет заднего фона скроллбара:')
	imgui.SameLine(nil, 3)
	if imgui.ColorEdit4(u8'Цвет заднего фона скроллбара', colorScrollbarBG, imgui.ColorEditFlags.NoInputs + imgui.ColorEditFlags.NoLabel) then
	...
	imgui.Text(u8'Цвет наведённого заднего фона скроллбара:')
	imgui.SameLine(nil, 3)
	if imgui.ColorEdit4(u8'Цвет наведённого заднего фона скроллбара', colorScrollbarHoveredBG, imgui.ColorEditFlags.NoInputs + imgui.ColorEditFlags.NoLabel) then
	...
	imgui.Text(u8'Цвет активного заднего фона скроллбара:')
	imgui.SameLine(nil, 3)
	if imgui.ColorEdit4(u8'Цвет активного заднего фона скроллбара', colorScrollbarActiveBG, imgui.ColorEditFlags.NoInputs + imgui.ColorEditFlags.NoLabel) then
	...
	imgui.Text(u8'Цвет перетаскивателя скроллбара:')
	imgui.SameLine(nil, 3)
	if imgui.ColorEdit4(u8'Цвет перетаскивателя скроллбара', colorScrollbarCursor, imgui.ColorEditFlags.NoInputs + imgui.ColorEditFlags.NoLabel) then
	...
	imgui.Text(u8'Цвет активного перетаскивателя скроллбара:')
	imgui.SameLine(nil, 3)
	if imgui.ColorEdit4(u8'Цвет активного перетаскивателя скроллбара', colorActiveScrollbarCursor, imgui.ColorEditFlags.NoInputs + imgui.ColorEditFlags.NoLabel) then
	...
	elseif msg == 257 then
		if openChat == true then
			if wparam == 0x0D --[[ENTER]] then
				...
				if text == '/chsettings' then enableSettingsWindow[0] = not enableSettingsWindow[0]
...
function splitsigned(n) -- СПАСИБО WINAPI.lua и GITHUB
	n = tonumber(n)
