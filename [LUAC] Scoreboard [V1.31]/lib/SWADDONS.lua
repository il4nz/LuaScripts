...
M.MaterialButton = function(str_id, size, duration)
	...
	local cols = {
		...
		active  = imgui.ImVec4(imgui.GetStyle().Colors[imgui.Col.Text]),
...
M.CloseButton = function(str_id, value, size, rounding)
	...
	local col = ToU32(imgui.GetStyle().Colors[imgui.Col.Text])
	local col_bg = hovered and 0xFFFFFF or 0xFFFFFF
	local offs = (size / 4)
